...
bool CMemoryHelper::LoadDLL(const std::string & szPath)
{
	...
	// Thread shell size
	auto dwThreadSize = (DWORD)UselessFunction - (DWORD)RemoteThreadRoutine;
	...
	// Thread param
	auto localThreadParam = reinterpret_cast<PREMOTE_THREAD_PARAM>(VirtualAlloc(NULL, sizeof(REMOTE_THREAD_PARAM), MEM_COMMIT | MEM_RESERVE, PAGE_EXECUTE_READWRITE));
	if (!localThreadParam) {
	...
	// Load shell code
	auto bShellCodeRet = LoadShellCode(RemoteThreadRoutine, dwThreadSize, localThreadParam, sizeof(REMOTE_THREAD_PARAM), 5000);
	if (!bShellCodeRet) {
