	...
	m_szLoggerName(szLoggerName), m_szFileName(szFileName)
{
	...
	{
		Logf(CUSTOM_LOG_FILENAME, XOR("There has to be an exception throw on CLogger::InitLogger(std::exception): %s\n"), ex.what());
		abort();
	}
...
CLogger::~CLogger()
{
	...
	{
		Logf(CUSTOM_LOG_FILENAME, XOR("Log destructor failed: %s\n"), ex.what());
		abort();
	}
}
...
void CLogger::sys_log(const char* c_szFunction, int iLevel, const char* c_szFormat, ...)
{
	...
	{
		Logf(CUSTOM_LOG_FILENAME, XOR("There has to be an exception throw on CLogger::sys_log(std::exception): %s\n"), ex.what());
		abort();
	}
	catch (DWORD dwNumber)
	{
